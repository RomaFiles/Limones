...
ip1 = 'rtsp://admin:@192.168.1.10:554/user=admin&password=&chanel=1&stream=0.sdp?real_stream'
ip2 = 'rtsp://admin:@192.168.1.20:554/user=admin&password=&chanel=1&stream=0.sdp?real_stream'
...
# Función para ajustar el brillo de un frame con GPU
def ajustarBrilloGPU(gpu_hsv_frame, brillo):
    # Aquí debes implementar la lógica para ajustar el brillo en GPU
    h, s, v = cv2.cuda.split(gpu_hsv_frame)
    v = cv2.cuda.add(v, brillo)
    return cv2.cuda.merge([h, s, v])
...
# Función para procesar video de cada cámara con GPU
def procesar_camara(cap, window_name):
    ...
    while True:
        ...
        # Convertir el frame a GpuMat
        gpu_frame = cv2.cuda_GpuMat()
        gpu_frame.upload(frame)

        # Procesamiento del frame en GPU
        gpu_hsv_frame = cv2.cuda.cvtColor(gpu_frame, cv2.COLOR_BGR2HSV)
        gpu_hsv_frame = ajustarBrilloGPU(gpu_hsv_frame, brillo)  # Ajustar brillo en GPU
        gpu_frame = cv2.cuda.cvtColor(gpu_hsv_frame, cv2.COLOR_HSV2BGR)

        # Convertir a escala de grises en GPU
        gpu_gray = cv2.cuda.cvtColor(gpu_frame, cv2.COLOR_BGR2GRAY)
        gpu_umbral = cv2.cuda.adaptiveThreshold(gpu_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, umbralNumero, 2)

        # Realizar el resto del procesamiento en GPU
        gpu_filtro = cv2.cuda.GaussianBlur(gpu_umbral, (7, 7), 0)
        gpu_filtro = cv2.cuda.morphologyEx(gpu_filtro, cv2.MORPH_OPEN, cv2.cuda_GpuMat(np.ones((3, 3), np.uint8)))

        # Encontrar bordes
        gpu_bordes = cv2.cuda.Canny(gpu_filtro, 150, 200)

        # Descargar bordes a CPU para encontrar contornos
        bordes_cpu = cv2.cuda.download(gpu_bordes)
        contornos, _ = cv2.findContours(bordes_cpu, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for contorno in contornos:
            ...
            if area > 2.0:
                ...
                # Crear una máscara en la CPU para calcular el color
        ...
        # Mostrar la imagen procesada (esto se realiza en CPU)
